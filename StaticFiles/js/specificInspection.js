...
export async function setup(node) {
  console.log("Showing Specific Inspection Item");
  ...
  if(token == null){
	...
	const uri = `https://localhost:7098/api/v1/inspection/${window.myGlobalVariable.taskId}`
    const options = {
		method: "GET",
		headers: {
    ...
    p1.innerHTML = `<h1>Update Inspection Record ${window.myGlobalVariable.taskId}</h1>`
    ...
    node.querySelector("form").addEventListener("submit", async () => {
        ...
        const putUri = `https://localhost:7098/api/v1/inspection/${window.myGlobalVariable.taskId}`
	    const putOptions = {
		method: "PUT",
		headers: {
			"Content-Type": "application/vnd.api+json",
