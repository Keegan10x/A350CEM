...
export async function setup(node) {
  ...
  if(token == null){
	...
	console.log(window.myGlobalVariable.serviceId)			      
	const uri = `https://localhost:7098/api/v1/services/${window.myGlobalVariable.serviceId}`
    const options = {
		method: "GET",
		headers: {
    ...
    p1.innerHTML = `<h1>Update Service Record ${window.myGlobalVariable.serviceId}</h1>`
    ...
    node.querySelector("form").addEventListener("submit", async () => {
        ...
        const putUri = `https://localhost:7098/api/v1/services/${window.myGlobalVariable.serviceId}`
	    const putOptions = {
		method: "PUT",
		headers: {
			"Content-Type": "application/vnd.api+json",
